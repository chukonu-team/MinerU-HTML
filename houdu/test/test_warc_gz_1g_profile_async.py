...
html_list = read_html_from_warc_gz('/root/data/CC-MAIN-20250918080014-20250918110014-00995.warc.gz')
end_time = time.time()
...
for i in range(0, len(html_list), batch_size):
    ...
    start_time = time.time()
    generate_outputs = dripper.generate_data(generate_inputs)
    end_time = time.time()
    elapsed_time = end_time - start_time
    elapsed_generate += elapsed_time
    batch_results = dripper.post_process_data(input_map, generate_inputs, process_datas, generate_outputs)
...
print(f"dripper.generate_data took {elapsed_generate:.2f} seconds")
pr.disable()
